...
module.exports = function(args) {
  ...
  const copyDirOptions = { ignoreHidden: !args.include_hidden };
  const staticJson = args.static_json ? pathFn.join(baseDir, args.static_json) : undefined;
  ...
  if (!args.repo && !args.repository) {
    ...
    help += '    include_hidden: false\n';
    help += '    static_json: <static.json file path>\n\n';
  ...
  function setup() {
    // Copy assets
    return fs.copyDir(assetDir, deployDir, copyDirOptions).then(() => {
      return git('init');
    }).then(() => {
      return git('add', '-A');
    }).then(() => {
  ...
  return fs.exists(deployDir).then(exist => {
    ...
    return fs.copyDir(publicDir, deployPubDir, copyDirOptions);
  }).then(() => {
    if (fs.existsSync(staticJson)) {
      log.info('Copying custom static.json config...');
      return fs.copyFile(staticJson, pathFn.join(deployDir, 'static.json'));
    }
  }).then(() => {
